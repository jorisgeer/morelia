...
//      bpos += mysnprintf(buf,bpos,blen,"\n  Tt%*s = %2u,\n\n  ",tknampad+1,"99_kwd",nkwd);
    ...
    for (tk = 0; tk < nltok; tk++) {
      ...
      spool[spos++] = '\\';
      spool[spos++] = '0';
      bpos += mysnprintf(buf,bpos,blen,"T%*s = %u,%s",tknampad,fmtname(len,nam),tk+nkwd,(tk & 7) == 7 ? "\n  " : " ");
      if (len == eoflen && memeq(nam,speceof,eoflen)) {
   ...
   if (rv == -1) return 1;
    ...
    myfprintf(&sfp,"enum %sToken {%s}; // gen.%u\n\n",packed8,buf,__LINE__);
    ...
    for (c = 0; c < 256; c++) {
      ...
      if (ctab[c] == nsets) { buf[bpos++] = 'X'; buf[bpos++] = ' '; buf[bpos++] = ' '; }
      else {
        bit = ctab[c];
        set = setmaps[bit];
        bpos += mysnprintf(buf,bpos,blen,"%-3s",strupper(setnams + set * Snam));
      }
    }
    myfprintf(&lhfp,"%s\n};\n#undef X\n",buf);
    printsets(&lhfp,0);
