...
// ------------
  ...
  if (argc > 1) {
    ...
    info("node %u argpos %u am %u ve %u",ni,napos0,am,ve);
    np->ve = ve;
    ...
    if (nxve < Ptablen) {
      ...
      if (repcnt > 1) { // create rep node
        ...
        for (rep = 0; rep < repcnt; rep++) {
          ...
          ve2 = np2->ve;
          aty = prd2nod[ve2];
          info("ve %u %s typ %u %s",ve,prdnam(ve,0),aty,atynam(aty));
          ndti = ndcnts[aty]++;
          nhtab[aid] = ndti | (aty << Atybit);
          ndargs[napos++] = aid++;
          ni2 = np2->sib;
...
//    icefln(FLN,hi32,fpos,"pop at lvl 0 after %u` tokens and %u` nodes",ti,ni);
  ...
  if (argc) {
    ...
    ndti = ndcnts[aty]++;
    sinfo(fpos,"ni %u argc %u t %u",ni,argc,aty);
...
// ---
  ...
  if (ti < tcnt) {
    info("ti %u/%u",ti,tcnt);
    ser(FLN,tk,0,fpos,ti,r,si,Ser_eof_nostart,T99_count,0,0);
    return 1;
  }
